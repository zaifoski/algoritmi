...
int minMax(coppia array[], int n, int &min, int &max){
  max = array[0].fine;
  min = array[0].inizio;
  for(int i = 1; i<n; i++){
    if(array[i].fine>max)
...
int main(){
  ...
  for(int i = min; i<=max; i++){
    ...
    if(!presente){//l'elemento è un candidato
      if(maxxTemp>0){//anche l'elemento precedente è un candidato
        maxxTemp++;
        nonPresenteTemp.fine = i+1;
      }
      else{//l'elemento è il primo dell'intervallo candidato
        if ((i-1)<0) nonPresenteTemp.inizio = 0;
        else nonPresenteTemp.inizio = i-1;
        nonPresenteTemp.fine = i+1;
        maxxTemp++;
      }
    }
    else{//l'elemento non è un candidato
      if(maxxTemp>0){//ma l'elemento prima sì
        if(maxxTemp>maxx){
          maxx = maxxTemp;
          //nonPresenteTemp.fine = i+1;
